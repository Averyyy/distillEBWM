...
class Transformer(nn.Module):
    ...
    def forward(self, embeddings: torch.Tensor, start_pos: int, learning = False):
        ...
        """
        if not learning:
            with torch.inference_mode(): #NOTE this is not used during validation or testing if using mcmc would only be used for like downstream classification or whatnot. cant use during val/test since need gradient
